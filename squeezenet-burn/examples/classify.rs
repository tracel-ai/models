...
fn main() {
    ...
    let image_input = Tensor::<3>::from_data(img_array, &device).reshape([1, 3, HEIGHT, WIDTH]);
    // Normalize the image
    ...
    #[cfg(feature = "weights_file")]
    let model = Model::from_file(weights_file.to_str().unwrap(), &device);

    #[cfg(feature = "weights_embedded")]
    // Load model from embedded weights
    let model = Model::from_embedded(&Default::default());

    // Run the model
    let output = model.forward(normalized_image);

    // Get the argmax of the output
    let arg_max = output.argmax(1).into_scalar::<i64>() as usize;
